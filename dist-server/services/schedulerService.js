...
export class SchedulerService {
    ...
    static async updateRecipientStatus(campaignId, recipientIndex, status, error) {
        // Mongoose specific update for array item
        const update = {};
        update[`recipients.${recipientIndex}.status`] = status;
        if (error)
            update[`recipients.${recipientIndex}.error`] = error;
        ...
        if (campaign) {
            ...
            if (error)
                campaign.recipients[recipientIndex].error = error;
            await storage.saveItem('scheduled_campaigns', campaign);
        }
    }
    static async updateProgress(campaignId, progress) {
        // Optimistic update
